...
import type { SkillColumn } from "@/lib/data"

export default function ExpertiseBlock({ columns }: { columns: SkillColumn[] }) {
  ...
  return (
    <section className="flex w-full items-center justify-center bg-background px-6 py-16 text-foreground">
      <div className="mx-auto w-full max-w-5xl">
        ...
        >
          {columns.map((column) => {
            const ColumnIcon = ICON_REGISTRY[column.icon];
            return (
              <motion.div layout key={column.status} className="flex flex-col gap-4">
                ...
                <motion.ul layout className="flex flex-col gap-4">
                  {column.items.map((item) => {
                    const TechIcon = ICON_REGISTRY[item.techIcon];
                    return (
                      <motion.li
                        key={item.title}
                        layout
