...
function ValueIndicator({
  ...
  if (displayStyle === "bubbles") {
    ...
    const finalWidth = cellWidth * (minScale + (1 - minScale) * scale);
    const finalHeight = cellHeight * (minScale + (1 - minScale) * scale);
...
export default function Heatmap(props: HeatmapProps) {
  ...
  const actualWidth = cellWidth ?? cellSize;
  const actualHeight = cellHeight ?? cellSize;
  ...
  const fontSize = Math.min(12, actualHeight);
  ...
  return (
    <div
      ...
      style={{
        gap,
        gridTemplateColumns: `max-content repeat(${weeks.length}, ${actualWidth}px)`,
        gridTemplateRows: `repeat(8, ${actualHeight}px)`
      }}
      {...safeProps}
    >
